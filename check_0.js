...
function renderPlayers() {
  ...
  playersBox.innerHTML = stats.length ? stats.map(p => {
    const url = `player.html?id=${encodeURIComponent(p.name)}`;
    return `<div class="player-card"><a href="${url}">${esc(p.name)}</a><div class="small" style="margin-top:8px">${p.wins}-${p.losses} • ${(p.pct*100).toFixed(1)}% • Diff ${p.diff} • Rating ${(p.elo||0).toFixed(1)} • ${p.playoffEligible ? "Playoff eligible" : "Below playoff threshold"}</div></div>`;
