...
function uploadPlayerPhoto_(payload){
  const name = String(payload.playerName || "player").trim() || "player";
  const dataUrl = String(payload.dataUrl || "");
  if (!dataUrl) throw new Error("Missing image data.");
  const parsed = parseDataUrl_(dataUrl);
  const folder = getPlayerPhotosFolder_(String(payload.folderId || ""));
  const fileName = sanitizeFileName_(name) + "-" + new Date().getTime() + ".jpg";
  const blob = Utilities.newBlob(parsed.bytes, parsed.mimeType || "image/jpeg", fileName);
