...
function doGet(e) {
  try {
    ...
    if (action === "ping") return dispatchAction_("ping", {});
    if (action === "loadLeagueState") return dispatchAction_("loadLeagueState", {});

    if (action === "updateWeekSignup") {
      const playersRaw = (e && e.parameter && e.parameter.players) || "";
      const players = String(playersRaw).split("|").map(function(x){ return String(x || "").trim(); }).filter(Boolean);
...
function dispatchAction_(action, payload){
  try {
    ...
    if (action === "saveLeagueState") return jsonOutput({ok:true, message:"League data saved to Google Sheets.", data:saveLeagueState_(payload)});
    if (action === "loadLeagueState") return jsonOutput({ok:true, data:loadLeagueState_()});
    if (action === "resetLeagueState") { resetLeagueState_(); return jsonOutput({ok:true, message:"Google Sheets league data was reset."}); }
    if (action === "updateMatchScore") return jsonOutput({ok:true, message:"Score updated successfully.", data:updateMatchScore_(payload)});
    if (action === "updateWeekSignup") return jsonOutput({ok:true, message:"Week sign-up saved successfully.", data:updateWeekSignup_(payload)});
    if (action === "uploadPlayerPhoto") return jsonOutput({ok:true, message:"Player photo uploaded successfully.", data:uploadPlayerPhoto_(payload)});
...
function getPlayerPhotosFolder_(folderId){
  if (folderId) {
    try { return DriveApp.getFolderById(folderId); } catch (err) {}
  }
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const folderName = "Pouncey Player Photos";
  const props = PropertiesService.getScriptProperties();
  const storedId = props.getProperty("POUNCEY_PLAYER_PHOTOS_FOLDER_ID");
  if (storedId) {
    try { return DriveApp.getFolderById(storedId); } catch (err) {}
  }
  const parentFiles = DriveApp.getFileById(ss.getId()).getParents();
  let parent = null;
  if (parentFiles.hasNext()) parent = parentFiles.next();
  let folder = null;
  if (parent) {
    ...
    folder = existing.hasNext() ? existing.next() : DriveApp.createFolder(folderName);
  }
  props.setProperty("POUNCEY_PLAYER_PHOTOS_FOLDER_ID", folder.getId());
  return folder;
}
...
function uploadPlayerPhoto_(payload){
  const playerName = String((payload && payload.playerName) || "").trim();
  const dataUrl = String((payload && payload.dataUrl) || "").trim();
  const folderId = String((payload && payload.folderId) || "").trim();
  const previousFileId = String((payload && payload.previousFileId) || "").trim();
  ...
  const safeName = sanitizeFileName_(playerName);
  const fileName = safeName + "_" + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyyMMdd_HHmmss") + "." + ext;

  const folder = getPlayerPhotosFolder_(folderId);
  ...
  if (previousFileId) {
    try {
      if (previousFileId !== file.getId()) DriveApp.getFileById(previousFileId).setTrashed(true);
    } catch (err) {}
  }

  return {
    photoUrl: "https://lh3.googleusercontent.com/d/" + file.getId(),
    photoDriveFileId: file.getId(),
