...
function doPost(e) {
  try {
    ...
    if (action === "saveLeagueState") {
      const data = saveLeagueState_(payload);
      return jsonOutput({ok:true, message:"League data saved to Google Sheets.", data:data});
    }
    ...
    if (action === "uploadPlayerPhoto") {
      const data = uploadPlayerPhoto_(payload);
      return jsonOutput({ok:true, message:"Player photo uploaded successfully.", data:data});
    }
...
function uploadPlayerPhoto_(payload){
  ...
  const safeName = playerName.replace(/[^A-Za-z0-9 _-]/g, "").trim() || "player";
